...
export const midiToFreq = (midi, a4 = 440) => a4 * Math.pow(2, (midi - 69) / 12);
...
// Click track plus a I-IV-V-I (major) / i-iv-v-i (natural minor, minor v) chord progression,
// one chord per bar, in the given key.
export function synthesizeProgression({ bpm, bars, beatsPerBar = 4, tonicPc, mode, sr = SR, a4 = 440, rng = null }) {
  ...
  const degrees = [1, 4, 5, 1]; // scale degree numbers, 1-indexed
  const chordSymbols = [];
  for (let bar = 0; bar < bars; bar++) {
    ...
    addChord(pcm, sr, bar * beatsPerBar * secondsPerBeat, beatsPerBar * secondsPerBeat, rootMidi, triadShape, 0.18, a4);
    chordSymbols.push({ startBar: bar, rootPc: (tonicPc + rootSemitone) % 12, quality: mode === 'major' ? 'maj' : 'min' });
  }
  if (rng) for (let i = 0; i < pcm.length; i++) pcm[i] += (rng() * 2 - 1) * 0.005;
