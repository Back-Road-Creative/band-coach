...
// for this whole directory. `pcm` is mono decoded audio as a plain Float32Array (decoding
// happens elsewhere — this module never touches AudioContext/File/fetch). Runs beat/tempo
// detection, beat-synchronous chroma, key and chord estimation, yielding control between
// stages (via microtask/macrotask breaks) so a caller running this inside a Worker can
// report progress without a single multi-second synchronous block. Returns:
//   { bpm, beats, downbeats, key, chords, tuningCents, confidence }
...
function emptyResult(confidence = 0) {
  return {
    bpm: 0,
    beats: [],
    downbeats: [],
...
export async function analyse(pcm, sampleRate, opts = {}) {
  ...
  if (!pcm || pcm.length < ONSET_FRAME_SIZE) {
    onProgress(1);
  ...
  if (beats.length === 0) {
    onProgress(1);
    return emptyResult(tempo.confidence);
  }
  ...
  // Tuning estimate from a sample of frames (every 4th, capped) against default A440, then
  // re-derive chroma per frame using the refined reference pitch.
  const sampleFrames = [];
  ...
  const chromaFrames = [];
  for (let f = 0; f < numChromaFrames; f++) {
    const mag = magStore.subarray(f * bins, (f + 1) * bins);
    chromaFrames.push(chromaFromSpectrum(mag, sampleRate, CHROMA_FRAME_SIZE, { a4 }));
  }
  onProgress(0.7);
  await yieldToEventLoop();
  ...
  const confidence = Math.max(0, Math.min(1, (tempo.confidence + key.confidence) / 2));
