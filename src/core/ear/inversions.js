// Chord-inversion recognition: triads root/1st/2nd, then sevenths with a
// 3rd inversion added.
//
// Wiring contract: call make(level, seed). Render `play` as one block chord
// through the app's synth. Collect the learner's answer as one of `choices`
// (a single inversion-name pick) and pass {question, response} to check().
...
export const LEVEL_NAMES = [
  ...
  'Triads, root and 1st inversion',
  'Triads, all inversions, all qualities',
  'Seventh chords, root and 1st inversion',
  'Seventh chords, all inversions',
];

// Rotate the lowest `inversion` chord tones up an octave, then re-sort -
// the standard mechanical definition of "Nth inversion", computed rather
// than looked up per quality.
...
function levelConfig(level) {
  if (level <= 1) {
    return { qualities: ['major', 'minor'], table: TRIAD_QUALITIES, names: TRIAD_INVERSION_NAMES, maxInversion: 0 };
  }
  if (level === 2) {
    return { qualities: ['major', 'minor'], table: TRIAD_QUALITIES, names: TRIAD_INVERSION_NAMES, maxInversion: 1 };
  }
  if (level === 3) {
    return {
      qualities: Object.keys(TRIAD_QUALITIES),
      table: TRIAD_QUALITIES,
      names: TRIAD_INVERSION_NAMES,
      maxInversion: 2,
    };
  }
  if (level === 4) {
    return {
      qualities: Object.keys(SEVENTH_QUALITIES),
      table: SEVENTH_QUALITIES,
      names: SEVENTH_INVERSION_NAMES,
      maxInversion: 1,
    };
  }
  return {
