...
// not at all). The learner says sharp, flat or same. Detuning tolerance
// shrinks as the level rises (50 cents down to 5).
//
// Wiring contract: call make(level, seed). Render `play` through the app's
// synth, applying each event's `cents` as a pitch-bend on top of its `midi`
// note (0 cents = untouched). Collect the learner's pick from `choices` and
// pass to check().
...
export function make(level, seed) {
  ...
  return {
    ...
    play: [
      { t: 0, dur: 1.0, midi: [midi], cents: 0 },
      { t: 1.2, dur: 1.0, midi: [midi], cents: centsOffset },
    ],
